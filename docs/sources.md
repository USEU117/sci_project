# 官方来源索引

## 方法

- AnomalyCLIP (ICLR 2024): <https://github.com/zqhang/AnomalyCLIP>
- WinCLIP (CVPR 2023) 论文: <https://arxiv.org/abs/2303.14814>
- WinCLIP 第三方复现: <https://github.com/caoyunkang/WinClip>
- PatchCore: <https://github.com/amazon-science/patchcore-inspection>
  - Tested HEAD commit: `fcaa92f124fb1ad74a7acf56726decd4b27cbcad`
  - Source ZIP SHA256: `17994A589AA979D2981153D57A8B1A7C354A57C803A2607239BF7C79A18B17F4`
- PromptAD (CVPR 2024): <https://github.com/FuNz-0/PromptAD>
- AnomalyDINO (WACV 2025): <https://github.com/dammsi/AnomalyDINO>
- ReMP-AD (ICCV 2025): <https://github.com/cshcma/ReMP-AD>
- ReMP-AD 论文: <https://openaccess.thecvf.com/content/ICCV2025/html/Ma_ReMP-AD_Retrieval-enhanced_Multi-modal_Prompt_Fusion_for_Few-Shot_Industrial_Visual_Anomaly_ICCV_2025_paper.html>
- AdaptCLIP (AAAI 2026): <https://github.com/gaobb/AdaptCLIP>
- AdaptCLIP 论文: <https://ojs.aaai.org/index.php/AAAI/article/view/42404>
- AdaptCLIP checkpoint: <https://huggingface.co/csgaobb/AdaptCLIP>

## 数据

- MVTec AD: <https://www.mvtec.com/research-teaching/datasets/mvtec-ad>
- VisA: <https://registry.opendata.aws/visa/>
- VisA 项目: <https://github.com/amazon-research/spot-diff>

所有资源下载后都应记录获取日期、URL、commit/checkpoint 名称与 SHA256。
