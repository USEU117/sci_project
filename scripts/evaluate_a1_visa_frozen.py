...
def evaluate_config(job: dict, map_size: tuple[int, int], dataset: str) -> dict:
    ...
    return {
        ...
        "stride": STRIDE,
        "dataset": dataset,
        "dataset_role": "in_domain_frozen_validation" if dataset == "visa" else "external_frozen_validation",
        "baseline_source": "feature_level_dino_only_knn (no v2 score cache on this dataset)",
