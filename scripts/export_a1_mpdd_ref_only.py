"""A1 reference-features-only export for MPDD (docs 阶段五 5.2).

Rationale: the A1 test patch features depend only on the (fixed) test images,
NOT on seed/shot. The k1 caches already contain the full test patch features.
For K=2/4 we only need NEW reference patch features (the memory-bank source
images change with shot). This script therefore:

  1. loads a base k1 feature cache (test fields reused verbatim),
  2. computes ONLY the reference patch features for the target (seed, shot),
  3. writes a new cache with identical schema + export_report.json.

This avoids re-running GPU inference on the entire test set 12 times (6
(seed,shot) combos x 2 branches), reducing GPU time and crash risk.

Branch support:
  --branch dino  -> DINOv2 (run in .venv-patchcore; model dinov2_vitb14 by default)
  --branch clip  -> AnomalyCLIP ViT-L/14@336 (run in .venv-anomalyclip; needs checkpoint)
"""
...
def main() -> int:
    ...
    parser.add_argument("--base-cache", type=Path, required=True,
                        help="existing k1 cache dir (same branch) whose test features are reused")
    ...
    parser.add_argument("--shot", type=int, choices=(1, 2, 4), required=True,
                        help="target shot; k1 base cache provides test features")
    ...
    with torch.inference_mode():
        for category, samples in sorted(indexed.items()):
            ...
            if ref_grid != test_grid:
                raise RuntimeError(
                    f"ref/test grid mismatch in {category}: {ref_grid} vs {test_grid}"
                )
