...
**否。** 12 个预注册候选在 MPDD seed0 × shot {1,2,4} 上的三-shot平均值全部低于 A1；36 个 candidate-shot 组合中 35 个下降，只有最佳候选的 s0/k1 微升 `+0.0003`，不具备实际意义：
...
小门范围内（MPDD seed0），12 个候选的三-shot平均值全部相对 A1 退化；36 个 candidate-shot 组合中 35 个退化，唯一正值为 `dino_to_clip_k5_lam0.25` 的 s0/k1 `+0.0003`。整体趋势：
- λ 越大（条件分数权重越高）退化越明显 → 该趋势与 `r_C|D` 在当前校准公式中是较弱异常证据的解释一致，但没有完整 λ=0 指标对照，不能把下降全部因果归因于条件项；
- shot 越大退化越明显（1→2→4）；
