...
| AnomalyDINO | DINOv2 (methods/anomalydino, official ViT-L/14) | official | 1/2/4-shot; few-shot fine-tuning | yes (few-shot fine-tuned) | yes (fine-tuning on normal references) | no | no | unified evaluator on project caches | v2 score prediction caches |
...
| WinCLIP+ | OpenCLIP ViT-B/16-plus-240 (project env) | official | zero-/one-shot; text-guided | none (zero-shot) | no | no | yes (text prompts) | unified evaluator on project caches | v2 score prediction caches |
...
- `PLAN.md`、`NEXT_ACTIONS.md`：PatchCore/AnomalyDINO/PromptAD/WinCLIP+/AnomalyCLIP 的 shot/seed/调优协议与 `target_normal_tuning=true` 标注
